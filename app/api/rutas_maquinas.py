...
def create_maquina():
    ...
    try:
        ...
        if not codigo:
            count = Maquina.query.count() + 1
            codigo = f"M-{count:03d}"
            while Maquina.query.filter_by(codigo=codigo).first():
                count += 1
                codigo = f"M-{count:03d}"
...
def update_maquina(id):
    ...
    try:
        if 'codigo' in data and data['codigo'] != maquina.codigo:
             exists = Maquina.query.filter_by(codigo=data['codigo']).first()
...
def get_tipos_maquina():
    tipos = TipoMaquina.query.order_by(TipoMaquina.nombre).all()
